...
def parse_xml_response(method: str, xml: str, result_cls: Type, is_array: bool = False) -> Any:
    """
    ...
    for child in root.iter():
        if child.tag.endswith(method + "Result"):
            result_element = child
            break

    if result_element is None:
        raise ValueError(f"No {method}Result found in response.")
